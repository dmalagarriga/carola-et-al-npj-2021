...
fout_all = open('Neurodegeneration_TH_neurite_retraction/Results_Analysis/Ratio_abrupt_events_all_SIMULATIONS_new.txt','w')
print >> fout_all,'CTR 0.142857142857 0.0124820458543'

for MUA in Exp_10_percent:
	fout_single = open('Neurodegeneration_TH_neurite_retraction/Results_Analysis/Ratio_abrupt_events_%s.txt' %(namestr(Exp_10_percent, globals())[0]) ,'w')
	Avg_maxima = []
...
for MUA in Exp_30_percent:
	fout_single = open('Neurodegeneration_TH_neurite_retraction/Results_Analysis/Ratio_abrupt_events_%s.txt' %(namestr(Exp_30_percent, globals())[0]) ,'w')
	Avg_maxima = []
	for d in range(len(MUA)):
		maxima = []
		minima = []
...
for MUA in Exp_50_percent:
	fout_single = open('Neurodegeneration_TH_neurite_retraction/Results_Analysis/Ratio_abrupt_events_%s.txt' %(namestr(Exp_50_percent, globals())[0]) ,'w')
	Avg_maxima = []
	for d in range(len(MUA)):
		maxima = []
		minima = []
