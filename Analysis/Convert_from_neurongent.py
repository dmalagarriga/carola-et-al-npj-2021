...
File_Dat1="%s/Kex1.dat" %Dir_Input
File_Dat2="%s/Kex2.dat" %Dir_Input
File_Dat3="%s/Kin.dat" %Dir_Input
File_Dat4="%s/Exc_Inh_List.dat" %Dir_Input
File_Dat5="%s/Dopaminergic_List.dat" %Dir_Input
File_Dat6="%s/Delay_Matrix.dat" %Dir_Input
File_Dat7="%s/Connections_all.dat" %Dir_Input
File_Dat8="%s/Number_of_Connections_all.dat" %Dir_Input
...
Connections = np.loadtxt('%s/cons10_new.txt' %Dir_Input, skiprows=9, unpack=False)

Prop_exc_inh=float(os.getenv('Prop_exc_inh'))

nDopaminergic = 0
nNeurons = int(len(Positions)) # Because Rewire_connections.py forgets the last neuron
...
Excitatory_list = random.sample(range(nNeurons-nDopaminergic),int((Prop_exc_inh)*(nNeurons-nDopaminergic)))
